##
####
###### LECTURE 2: GGPLOT VERSION
#### Introduction_to_statistics:bioinformatics.R
##

# This script is absolutely the same as Introduction_to_statistics:bioinformatics.R
# but using another library called ggplot2 to make the graphs.
# ggplot belongs to a collection of packages called tidyverse which
# modify the standard R syntax and make it more user-friendly
...
data <- readRDS("~/Desktop/ds_sessions_2_data.rds")
...
# --- Barplot ----
# Instead of using base R barplot, we will use the ggplot library

# The first thing we need to make a ggplot graph is a data.frame with "tidy" data
# Tidy data means that the data is organized in a way that each row is an observation
# of a series of variables in the columns

# The original R code would be:
# barplot(table(metadata$gender), ylim = c(0, 80), main = "Gender", ylab = "Frequency", 
#        xlab = "Gender", col = c("blue", "yellow"))

# Instead, we will do: 
ggplot(data=metadata) + # "start a ggplot object"
  geom_bar( aes(x=gender) ) # "grab the column gender and produce a barplot out of it"

# The aes() function takes as an input a column of the dataframe specified in data,
# and depending on the type of data that it contains, it will map the specified
# *aesthetic* to a certain variable. Here the aesthetic is the 
# distribution across the x axis and variable is gender

# Adding a bit more aesthetic options
ggplot(data=metadata) +
  geom_bar( aes(x=gender, fill=gender) ) + # "grab the column gender and fill the bars accoring to it"
  labs( # specify plot labels: title, x and y
    title = "Distribution of gender among patients",
    y = "Number of patients",
    x = "Gender"
  )

# With ggplot you don't have to specify the colors by hand,
# it automatically chooses a palette based on the variable you tell it to use
...
# Error: StatBin requires a continuous x variable: 
# the x variable is discrete.Perhaps you want stat="count"?

# The reason for this it that age has been saved as a character vector, instead of numeric
class(metadata$age)
metadata$age = as.numeric(metadata$age) 
# Fixed! Now back to the histogram
ggplot(metadata) +
  geom_histogram(aes(x = age))

# Again, let's add decorations
ggplot(metadata) +
  geom_histogram(
    aes(x = age),
    binwidth = 10,
    # You could specify colors by hand also if you wanted, 
    # although it is generally better to let ggplot do it for you
    fill = c("red", "yellow", "blue", "green", "pink", "purple", "darkgreen")
    ) + 
...
# Let's redefine expr as it used to be
expr <- as.data.frame(exprs(data[[1]]))

# ---- Visualization for numerical variables ----
# In the case of numerical variables there are multiple alternatives on how to 
# visualize your data. Here are the most relevant and probabaly the only ones 
# you will have to use. 


# ----- Boxplot ------
# Base R: boxplot(expr$GSM5001936)
# ggplot: 
ggplot(expr, aes(x=GSM5001936)) + # You can also specify the aes() at the beginning
  geom_boxplot()
...
ggplot(expr) +
  geom_boxplot(aes(x=GSM5001936)) +
  # we specify limits with scale_x_continuous()
  scale_x_continuous(limits = c(0, 1500)) + 
  ggtitle("Sample GSM5001936") # Another option if you only want to set the title

# Logarithmic scale
# boxplot(log2(expr$GSM5001936), horizontal=T)
ggplot(expr) +
  geom_boxplot(aes(x = log2(GSM5001936))) +
  ggtitle("Sample GSM5001936") 
...
# qqnorm(expr[1,]) #qqline(expr[1,])

expr_t = as.data.frame(t(expr))
gene1 = colnames(expr_t)[1]
ggplot(expr_t, aes(sample = get(gene1))) + 
  geom_qq() +
  geom_qq_line()
...
ggplot(expr_final, aes(x=X1007_s_at, color = condition)) + 
  geom_density() + 
  # don't worry too much about this, I had to copy it from stack overflow so
  stat_function(fun = dnorm, #returns the probability (i.e., the density) for a given value from a normal distribution of known mean and standard deviation
                args = list(mean = mean(expr_final$X1007_s_at),
                            sd = sd(expr_final$X1007_s_at)),
...
healthy = expr_final[expr_final$condition == "normal", 1:ncol(expr_final)-1]
tumor =expr_final[expr_final$condition == "colorectal cancer", 1:ncol(expr_final)-1]

pvalue = NULL
for(i in 1 : ncol(healthy)) { 
  x = healthy[,i] 
  y = tumor[,i] 
  t = t.test(x, y)
...
# First a dataframe, then the plot
vulcano_df = data.frame(logFC = log2(fold_change),
                        neg_log_pval = -log10(pvalue))

ggplot(vulcano_df, aes(x = logFC, y = neg_log_pval)) + 
  geom_point() + 
  geom_vline(xintercept = c(log2(fold_threshold), 
                            -log2(fold_threshold)),
             color = c("red","blue")
             ) + 
...
diag(corr) = NA
pheatmap::pheatmap(corr) # This package is way better than ggplot for heatmaps
